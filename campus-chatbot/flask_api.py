...
try:
    from campus_navigator_groq import CampusNavigator
except Exception:
    try:
        from campus_navigator import CampusNavigator
    except Exception:
        raise ImportError(
            "Could not import CampusNavigator. Make sure one of: 'campus_navigator_groq.py' or 'campus_navigator.py' exists and is importable."
        )
...
@app.route('/api/debug/last_llm', methods=['GET'])
def debug_last_llm():
    """Return the last LLM payload recorded by the navigator (local dev only)."""
    try:
        # Only allow localhost requests for safety
        ...
        last = None
        try:
            last = getattr(navigator, '_last_llm', None)
        except Exception:
            last = None

        return jsonify({'success': True, 'last_llm': last}), 200
    except Exception as e:
...
if __name__ == '__main__':
    ...
    print("=" * 50)
    # Helpful runtime hints for LLM configuration
    try:
        groq_model = os.environ.get('GROQ_MODEL')
        groq_key = os.environ.get('GROQ_API_KEY')
        print("\nLLM Configuration Hints:")
        print(f"  GROQ_MODEL = {groq_model if groq_model else '<not set; default will be used>'}")
        print(f"  GROQ_API_KEY = {'<set>' if groq_key else '<not set>'}")
        print("\nSet these in PowerShell for the current session, e.g.:")
        print("  $env:GROQ_API_KEY = 'sk-...'")
        print("  $env:GROQ_MODEL = 'openai/gpt-oss-20b'  # optional")
    except Exception:
        pass
